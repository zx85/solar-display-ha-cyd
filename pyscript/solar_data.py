...
def get_solar_data():
    states={}
    state_list={"solar_in":"sensor.solis_ac_output_total_power",        # current solar power
    ...
    states['timestamp']=state.getattr(sensor.solis_total_consumption_power)['Last updated']
    states['bins']=state.getattr(sensor.upcoming_bins)['bins']
    presence_string=''
    for person in ['james']:
        if state.get(f'device_tracker.{person.lower()}_phone') == 'home':
            presence_string+=(person[:1]).replace('L','LE') # Shim for name changes
